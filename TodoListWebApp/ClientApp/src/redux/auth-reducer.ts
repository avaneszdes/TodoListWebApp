...
import {AUTHORIZATION_SUCCEED, EDIT_USER_PHOTO, LOG_OUT, REGISTRATION} from "./constants";
...
const auth = (state = initialState, action: TodosActionTypes) => {
    ...
    switch (action.type) {
        ...
        case AUTHORIZATION_SUCCEED:
            return {token: action.payload.token, role: action.payload.role, name: action.payload.name, id: action.payload.id, photo: action.payload.photo}

        case LOG_OUT:
            return {token: action.payload, role: action.payload, name: action.payload}

        case EDIT_USER_PHOTO:
            console.log({...state, photo: action.payload.photo, id: action.payload.id})
            console.log(state)
